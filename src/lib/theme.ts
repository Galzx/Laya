...
export function generateCustomThemeTokens(options: {
  ...
  if (mode === "light") {
    ...
    return {
      ...
      border: hslToCssString(baseH, Math.max(8, baseS - 4), Math.max(76, baseL - 12)),
      input: hslToCssString(baseH, Math.max(8, baseS - 4), Math.max(76, baseL - 12)),
      ring: hslToCssString(accH, accS, accL),
      sidebar: hslToCssString(baseH, sidebarS, sidebarL),
      sidebarBorder: hslToCssString(baseH, Math.max(8, sidebarS - 4), Math.max(72, sidebarL - 8)),
    };
  } else {
